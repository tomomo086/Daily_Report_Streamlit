...
class PatrolTimeGenerator:
    ...
    def generate_4post_times(self, patrol_start, large, medium, small):
        """4ポストの巡回時間を生成"""
        if patrol_start == "23:00頃":
            return self._generate_4post_23(large, medium, small)
        elif patrol_start == "22:00頃":
            return self._generate_4post_23(large, medium, small)  # 22:00も23:00にスライド
        else:  # 21:00頃 or その他
    ...
    def _generate_4post_23(self, large, medium, small):
        """23:00頃開始の4ポスト巡回時間（22:00の1時間スライド）"""
        start_time = datetime.strptime("23:00", "%H:%M") + timedelta(minutes=random.randint(0, 10))
        records = []
        patrol_order = [
            ('C17', 'E17', 5),
            ('C18', 'E18', 5),
        ...
        comments = self._get_comments_23(large, medium, small)
        current_time = start_time
        for i, (start_cell, end_cell, duration) in enumerate(patrol_order):
    ...
    def _get_comments_23(self, large, medium, small):
        """23:00頃開始時のコメント取得（22:00の1時間スライド版）"""
    ...
    def generate_5post_times(self, large, medium, small):
        """5ポストの巡回時間を生成"""
        start_time = datetime.strptime("23:00", "%H:%M") + timedelta(minutes=random.randint(0, 5))
        records = []
        # 1回目の巡回
    ...
    def generate_other_times(self):
        ...
        return {
            ...
            'morning_4post_2': (datetime.strptime("7:30", "%H:%M") + 
                              timedelta(minutes=random.randint(0, 20))).strftime("%H:%M").lstrip("0"),
            'night_4post': (datetime.strptime("23:00", "%H:%M") + 
                          timedelta(minutes=random.randint(0, 10))).strftime("%H:%M").lstrip("0"),
            'patrol_4post': (datetime.strptime("22:30", "%H:%M") + 
                           timedelta(minutes=random.randint(0, 3))).strftime("%H:%M").lstrip("0"),
            'patrol_4post_end': (datetime.strptime("23:50", "%H:%M") + 
